...
def send_confirm_rescheduling_to_client(self, user_id: str, scheduling_id: uuid.UUID) -> None:
    """
    Envia o e-mail de confirmação de reagendamento ao cliente.
    ...
    try:
        ...
        send_html_email(
            ...
            to_email=user.email,
            template_name="services/emails/confirm_rescheduling_client.html",
            context=context,
        )

        logger.info("Rescheduling confirmation email sent to %s (scheduling=%s)", user.email, scheduling.id)

    except Exception as exc:
        logger.exception("Error sending rescheduling confirmation email (user=%s, scheduling=%s)", user_id, scheduling_id)
