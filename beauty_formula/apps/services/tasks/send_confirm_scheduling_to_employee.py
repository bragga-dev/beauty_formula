...
def send_confirm_scheduling_to_employee(self, scheduling_id: uuid.UUID) -> None:
    """
    ...
    try:
        ...
        send_html_email(
            subject="Novo agendamento na sua agenda — Fórmula da Beleza",
            to_email=employee_email,
            template_name="services/emails/confirm_scheduling_employee.html",
            context=context,
        )

        logger.info(
            "Scheduling confirmation email sent to employee %s (scheduling=%s)", employee_email, scheduling.id
        )

    except Exception as exc:
        logger.exception(
            "Error sending scheduling confirmation email to employee (scheduling=%s)", scheduling_id
        )
